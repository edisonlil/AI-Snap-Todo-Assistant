...
- 功能点远程选择与搜索体验增强：
  - 控制面板中的功能点选择新增分页搜索、无限滚动和滚动预加载能力，提升大结果集下的加载与检索体验。
  - 功能点搜索新增按产品线过滤，支持在控制面板中结合产品线进一步收敛候选结果。
  - 远程选择字段补充手动输入提交、结果合并去重和空查询状态清理，改善复杂表单场景下的可用性与稳定性。
- 组合框与截图显示细节优化：
  - 优化组合框只读模式下的光标、选中和点击展开行为，提升工单筛选等只读下拉场景的交互一致性。
  - 修复 macOS 透明截图的 alpha 处理与预览暗边问题，改善截图预览显示效果。
- 测试与打包配置补充：
  - 为功能点分页加载、产品线过滤、只读下拉框和透明图像处理补充测试覆盖。
  - 调整 macOS 打包配置，保持应用标准 Dock 图标显示。

### 2026-07-03

- 待办详情结论模式与时间线同步增强：
  - 新增待办详情“结论模式”配置选项，支持按场景调整结论展示方式。
  - 待办详情补充 `issue_product` 字段支持，控制面板同步完善问题产品字段的路径标准化与缓存处理。
  - 修复结论为空时时间线事件不显示、结论时间线展示不同步等问题，提升详情与时间线的一致性。

### 2026-07-02

- 控制面板表单能力增强：
  - 新增问题重现概率字段，补充功能点选项搜索能力。
  - 为 QML 下拉选择组件加入模糊搜索过滤，提升大选项集下的检索效率。

### 2026-07-01

- 项目版本管理重构：
  - 重构本地存储中的项目版本管理逻辑，为后续项目与工单字段扩展提供更稳定的数据基础。

### 2026-06-30

- 项目快照与产品字段链路增强：
  - 重构控制面板中的项目快照详情提取逻辑，提升快照读取与展示的稳定性。
  - 待办详情新增问题归属产品字段支持，并优化项目产品线逻辑。
  - 扩展工单存储字段与界面选项加载流程，为更多业务字段提供支撑。

### 2026-06-29

- 案例搜索与附件摘要体验优化：
  - 简化案例搜索实现并切换到服务端工作流，统一检索处理链路。
  - 新增当前摘要附件支持，优化待办附件管理界面交互，改善摘要与附件的协同编辑体验。

### 2026-06-28

- 客户环境与控制面板交互补强：
  - 新增客户环境字段支持，完善工单与待办上下文信息。
  - 优化项目与票据数据加载及辅助窗口创建逻辑，减少控制面板初始化和切换时的额外负担。
  - 修复下拉弹窗定位及客户环境编辑重试问题，提升表单交互稳定性。

### 2026-06-27

- 错误码查询能力上线：
  - 集成 Chattodo 服务端错误码查询功能，便于在待办处理流程中快速检索问题码信息。

### 2026-06-26

- 同步状态配置补充：
  - 新增待办同步状态显示配置选项，支持按需展示同步状态信息。

### 2026-06-25

- 待办详情布局与项目匹配能力优化：
  - 重构待办详情面板字段布局，改善信息分区与阅读效率。
  - 新增项目匹配候选功能和搜索实现，提升项目关联录入效率。

### 2026-06-16

- 待办栏交互与同步能力增强：
  - 优化待办栏与 `TodoPanel` 的联动逻辑，补强列表刷新、场景切换和内容回填时的状态同步表现。
  - 调整工具栏与待办面板之间的数据更新链路，减少分析结果落库、列表展示和面板状态不一致的情况。
- 分析策略与存储链路补强：
  - 更新分析意图与策略判定逻辑，补充更稳定的规则处理路径。
  - 完善服务端接口、SQLite 仓储与待办存储的细节处理，提升待办状态落盘与恢复的一致性。
- 测试覆盖扩展：
  - 新增待办栏、场景组合、待办存储、服务端接口和分析策略相关测试，覆盖近期交互和数据链路调整。

### 2026-06-15

- 图片原地翻译能力上线：
  - 新增截图区域内的图片翻译入口，支持结合 OCR 与服务端接口完成原图文字识别、翻译与结果回写。
  - 工具栏、主流程与截图交互同步接入图片翻译能力，便于在截图后直接发起翻译处理。
  - 补充图片翻译与服务端接口测试，并更新打包依赖与资源文件，保证新功能可随构建产物一并分发。
- 时间线详情编辑能力增强：
  - 新增 `TimelineDetailWindow`，支持对时间线详细记录进行编辑、预览和内容补充。
  - 扩展待办详情面板和默认时间线卡片的联动能力，完善详细记录的查看与保存流程。
- 分析与界面链路精简：
  - 移除未再使用的焦点提示弹窗和相关策略分支，简化截图分析流程中的中间交互。
  - 同步调整工具栏与分析意图逻辑，减少无效提示并降低维护成本。

### 2026-06-14

- TodoPanel 迷你模式与悬停交互上线：
  - 为待办栏新增迷你模式、停靠端状态和悬停展开交互，在收起状态下保留更轻量的桌面占用。
  - 增强 Python 后端与 QML 面板的属性、信号和动画配合，使面板在最小化与展开之间切换更顺滑。
- 桌面平台集成增强：
  - 新增 macOS Dock 菜单和 Windows 任务栏任务支持，并通过本地命令通道把快捷操作转发到运行中的应用程序。
  - 控制面板补充 macOS 自绘交通灯按钮，改进不同桌面平台上的窗口控制一致性。
  - 增强应用激活处理逻辑，避免鼠标停留在应用窗口时被外部激活误触发控制面板显示。
- 详情面板与主题样式优化：
  - 优化待办详情面板的完成条件与提示文案，确保问题结论填写后再允许完成操作。
  - `EnvironmentAccessPopover`、`TodoDetailPanel` 及控制面板相关区域切换到共享主题令牌，统一间距、宽度和视觉样式。
- 测试补强：
  - 补充主流程、任务栏集成、迷你面板、详情面板与主题令牌相关测试，覆盖跨平台与交互状态回归。

### 2026-06-13

- 控制面板结构重构：
  - 控制面板默认聚焦服务器配置区域，隐藏模型与分析规则部分，简化常用配置入口。
  - 项目、环境和工单区域逐步切换到 `PageRuntime`、`DetailRuntime` 等新组件，理顺布局结构与可见性逻辑。
- 服务端接入流程收敛：
  - 新增服务器登录能力，要求显式填写服务端地址与 API Key，明确连接前置条件。
  - 为 Windows 增加应用用户模型 ID 与图标设置，改善任务栏与系统层面的应用识别效果。
- 视觉规范统一：
  - 统一按钮高度、圆角、边框与表单控件样式，调整环境列表与多个控制面板分区的布局细节，提升整体界面一致性。
- 测试同步更新：
  - 新增或更新控制面板、运行时组件、主题样式和主流程相关测试，覆盖新的结构与表单行为。

### 2026-06-12

- 主题管理能力上线：
  - 引入主题管理与主题控制器，统一通知、结果确认、控制面板、待办详情和待办栏等界面的主题配置。
  - 将控制面板表单样式沉淀为可复用组件与主题令牌，减少不同页面之间的样式分叉。
- 控件与表单体验优化：
  - 新增像素输入等表单能力，持续打磨输入框、分段按钮、日期字段和按钮样式。
  - 修复群名显示异常问题，改善项目与控制面板中的信息回显表现。
- 待办确认流程简化：
  - 移除工地待办确认底部的反馈修正栏，减少重复输入区域，让确认面板职责更聚焦。
- 测试覆盖补强：
  - 补充主题、表单令牌、待办详情与结果确认相关测试，确保样式和交互调整具备回归保障。

### 2026-06-11

- 工单管理列表体验优化：
  - 重构工单管理列表与页面运行时布局，提升列表展示密度、信息组织和滚动浏览体验。
  - 为工单区域补充新的页面运行时支撑，便于后续保持不同业务分区的一致布局行为。
- 截图分析交互精简：
  - 移除不再需要的独立加载弹窗，简化截图分析过程中的过渡界面，减少等待期干扰。
- 控制面板历史逻辑清理：
  - 删除未使用的 `LLMService` 和相关验证逻辑，降低控制面板状态维护复杂度。
- 开发环境支持补充：
  - 新增 `devcontainer` 配置，为 DevPod 等远程开发环境提供更直接的工程启动支持。
