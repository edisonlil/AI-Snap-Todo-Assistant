...
- 首次在 macOS 使用全局截图热键时，可能需要在“系统设置 > 隐私与安全性”中为终端或打包后的 `Chattodo.app` 开启“辅助功能”和“输入监听”权限。
- 如果权限未开启，Chattodo 会继续启动，并保留菜单栏入口；授权后重启应用即可重新启用热键。
...
- ACH 状态同步能力增强：
  - 控制面板新增缺失 ACH 状态的异步刷新链路，可批量从服务端获取并校准工单 ACH 状态。
  - 服务端 API 新增 ACH 状态批量查询方法，工单同步逻辑补齐缺失 ACH 信息的回填处理。
- 工单详情与列表体验调整：
  - 待办详情页新增 ACH 单号只读展示，便于查看已同步的 ACH 信息。
  - 移除工单信息复制功能及相关 UI 元素，简化工单列表操作入口。
- 测试与文档同步：
  - 更新控制面板、服务端 API、工单同步和 `TicketsSection` QML 测试，覆盖 ACH 状态刷新与复制入口移除后的行为。
  - 同步精简产品功能树中已移除的工单复制描述。

### 2026-05-28

- 品牌命名统一为 Chattodo：
  - 将应用展示名称、Windows 版本信息、PyInstaller spec、构建脚本和云端构建工作流中的产物命名统一切换为 `Chattodo`。
  - 保留 `aica` 包名、配置目录和工程入口等内部技术标识，避免影响既有运行环境与开发流程。
- 构建产物管理优化：
  - 为 macOS unsigned 和 Windows exe GitHub Actions 构建产物增加保留天数配置，减少历史构建产物长期堆积。
- 桌面端细节修复：
  - 优化全局热键显示名称，减少系统快捷键展示中遗留旧名称的情况。
  - 修复待办详情中的产品线展示细节，并补充热键与待办详情相关测试覆盖。

### 2026-05-27

- 工单同步与远程附件能力增强：
  - 新增控制面板工单同步入口，支持从服务端同步工单并下载远程附件到本地。
  - 扩展服务端 API、SQLite 仓储和工单同步链路，补齐附件、状态与同步结果的持久化处理。
- 附件与文件对象支持完善：
  - 新增文件对象 ID 存储与传递能力，便于截图分析、日志分析和工单详情复用服务端文件。
  - 修复图片附件在时间线和待办详情中的显示问题，补充上下文摘要与附件展示相关测试。
- 产品线选择体验优化：
  - 优化待办详情里的产品线选择交互，支持从项目与服务端数据中更稳定地保留和回填产品线信息。
  - 补充控制面板、服务端 API、待办详情、待办存储与工单同步测试覆盖。

### 2026-05-26

- 服务端优先分析与工单集成上线：
  - 截图分析流程改为服务端优先策略，服务端不可用时保留本地分析回退能力。
  - 新增工单领域事件发布与服务端集成链路，将待办创建、更新等关键事件同步到远端。
- 工单字段与列表体验增强：
  - 新增 ACH 字段支持，并优化 `TicketsSection` 的界面样式与展示细节。
  - 扩展服务端 API、路径处理、单实例消息和主流程集成，支撑工单同步后的稳定运行。
- 附件展示修复与测试补强：
  - 修复图片附件在默认时间线卡片和待办详情面板中的展示问题。
  - 新增分析策略、工单同步、服务端 API 与待办详情相关测试，覆盖服务端优先和回退路径。

### 2026-05-25

- 工单配置与服务端工作流调整：
  - 移除本地功能点配置，改为使用服务端工作流和根因选项。
  - 优化工单丰富化逻辑，使结论生成、失败处理和服务端选项加载保持一致。
- 产品线选择与项目同步增强：
  - 在结果确认弹窗中新增产品线选择下拉框，支持从项目和已有待办信息中回填产品线。
  - 项目同步时不再覆盖已有产品版本信息，避免远端数据刷新破坏本地已确认版本。
- 打包辅助与测试整理：
  - 移除已不再使用的 `chattodo-cross-packager` 本地 skill 目录。
  - 补充结果弹窗、项目同步、待办详情、待办存储、控制面板和工单丰富化相关测试覆盖。

### 2026-05-24

- Chattodo Windows 云端打包流程上线：
  - 新增 GitHub Actions Windows exe 构建工作流，支持手动触发和推送 `v*` tag 时自动构建。
  - 构建产物同时覆盖 `onedir` 与 `onefile` 两种 Windows 包，分别上传为 `windows-onedir-zip` 和 `windows-onefile-exe`。
- 服务端配置与同步能力增强：
  - 新增服务端配置入口，为后续服务端同步提供统一连接参数。
  - 群聊保存后会推送到服务端。
  - 项目保存版本后会自动推送到服务端。
- 工单列表与刷新体验优化：
  - 优化截图入口和工单刷新链路。
  - 工单列表支持横向滚动。
  - 工单列表行支持点击进入详情页，并补充对应 QML 测试覆盖。
- 待办确认框与窗口体验修复：
  - 修复待办确认框输入空格时光标跳动的问题。
  - 待办确认面板支持拖动。
  - 去掉 Qt 外层 12px 透明 margin，并禁用 Windows DWM 为无边框窗口补上的边框线。
