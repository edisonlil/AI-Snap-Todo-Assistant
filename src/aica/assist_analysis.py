...
def build_assist_analysis_cache_key(todo_id: str, todo_payload: object) -> str:
    ...
    for item in _timeline_items(payload.get("timeline")):
        digest.update(b"\0")
        digest.update(sanitize_text(item.get("id")).strip().encode("utf-8", errors="ignore"))
        digest.update(sanitize_text(item.get("timestamp")).strip().encode("utf-8", errors="ignore"))
        digest.update(sanitize_text(item.get("content")).strip().encode("utf-8", errors="ignore"))
...
def _timeline_items(value: object) -> list[dict[str, object]]:
    ...
    for item in list(value or []) if isinstance(value, list) else []:
        if isinstance(item, TimelineEvent):
            result.append({"id": item.id, "timestamp": item.timestamp, "content": item.content})
